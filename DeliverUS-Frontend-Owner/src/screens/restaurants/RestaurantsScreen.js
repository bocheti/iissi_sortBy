...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const list = ['order', 'name', 'description', 'price', 'restaurantId', 'productCategoryId']
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        <TextSemiBold>Currently sorting by <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>{list[item.sortBy]}</TextSemiBold></TextSemiBold>
        ...
        <Pressable
          ...
          <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
            <MaterialCommunityIcons name='trending-up' color={'white'} size={20}/>
            <TextRegular textStyle={styles.text}>
              Sort by {item.sortBy === 5 ? 'order' : list[item.sortBy + 1]}
            </TextRegular>
          </View>
        </Pressable>
        </View>
  ...
  const sort = async (restaurant) => {
    try {
      const newRes = await toggleSort(restaurant.id)
      await fetchRestaurants()
      showMessage({
        message: `Restaurant ${newRes.name} now sorting products by ${list[newRes.sortBy]}`,
        type: 'success',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
      ...
      setRestaurantToBeDeleted(null)
      showMessage({
        message: `Restaurant ${restaurant.name} still sorting products by ${list[restaurant.sortBy]}`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
